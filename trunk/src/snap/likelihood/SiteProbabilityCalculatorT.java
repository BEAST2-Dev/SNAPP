...
//					System.out.println("Nb["+n+"] = "+rootData.m_Nb[n]);
	...
	static FCacheT [] m_cache; 

	public void clearCache(int nNodeNrMax, int nRedsMax, int nThreads) {
	...
	void doCachedInternalLikelihood(NodeData u1, NodeData u2, NodeData parent, int iThread) {
		FCache.CacheObject o = m_cache[iThread].getBottomOfBrancheF(u1.getCacheIDT(), u2.getCacheIDT(), u1, u2, parent, this);
	...
	void doCachedTopOfBranchLikelihood(NodeData node, double u, double v, double rate, Double [] coalescenceRate, int iThread) throws Exception {
		FCache.CacheObject o = m_cache[iThread].getTopOfBrancheF(node.getCacheIDB(), node, u, v, rate, coalescenceRate, this);
...
//			node.initFb(node.m_n, nReds);
	 ...
	 **/
	void doTopOfBranchLikelihood(NodeData node, double u, double v, double rate, Double [] coalescenceRate, boolean dprint) throws Exception {
		...
		FMatrix tmp = MatrixExponentiator.expQTtx(node.getSize(), u, v, coalescenceRate[node.getNr()], rate * node.t(), node.getFb()); 
	...
	void computeSiteLikelihood2(NodeData tree, double u, double v, double rate, Double [] coalescenceRate, int [] redCount, int [] lineageCount, boolean bHasDominantMarkers, boolean dprint/*=false*/) throws Exception {
		...
		if (tree.isLeaf()) {
			...
			computeSiteLikelihood2(p, u, v, rate, coalescenceRate, redCount, lineageCount, bHasDominantMarkers, dprint);
			doTopOfBranchLikelihood(p, u,v, rate, coalescenceRate,dprint);
			...
			computeSiteLikelihood2(leftChild, u, v, rate, coalescenceRate, redCount, lineageCount, bHasDominantMarkers, dprint);
			computeSiteLikelihood2(rightChild, u, v, rate, coalescenceRate, redCount, lineageCount, bHasDominantMarkers, dprint);
			doTopOfBranchLikelihood(leftChild, u,v, rate, coalescenceRate,dprint);
	...
	void computeCachedSiteLikelihood2(NodeData tree, double u, double v, double rate, Double [] coalescenceRate, int [] redCount, int [] lineageCount, boolean bHasDominantMarkers, boolean dprint/*=false*/, int iThread) throws Exception {
		//Post-order traversal
		if (tree.isLeaf()) {
			doCachedLeafLikelihood(tree, redCount[tree.getNr()], lineageCount[tree.getNr()], bHasDominantMarkers, iThread);
		} else if (tree.getNrOfChildren() == 1) {
			NodeData p = tree.getChild(0);
			computeCachedSiteLikelihood2(p, u, v, rate, coalescenceRate, redCount, lineageCount, bHasDominantMarkers, dprint, iThread);
			doCachedTopOfBranchLikelihood(p, u,v, rate, coalescenceRate, iThread);
			doInternalLikelihood(p, tree, false);
		} else { // assume two children
			NodeData leftChild = tree.getChild(0);
			NodeData rightChild = tree.getChild(1);
			computeCachedSiteLikelihood2(leftChild, u, v, rate, coalescenceRate, redCount, lineageCount, bHasDominantMarkers, dprint, iThread);
			computeCachedSiteLikelihood2(rightChild, u, v, rate, coalescenceRate, redCount, lineageCount, bHasDominantMarkers, dprint, iThread);
			doCachedTopOfBranchLikelihood(leftChild, u,v, rate, coalescenceRate, iThread);
			doCachedTopOfBranchLikelihood(rightChild, u,v, rate, coalescenceRate, iThread);
			doCachedInternalLikelihood(leftChild, rightChild, tree, iThread);
		}
	} // computeSiteLikelihood2
	
	public double computeSiteLikelihood(NodeData tree, double u, double v, double rate, Double [] coalescenceRate, int [] redCount, int [] lineageCount, boolean bMutationOnlyAtRoot, boolean bHasDominantMarkers, boolean useCache, boolean dprint/*=false*/, int iThread) throws Exception {
		...
		if (useCache) {
			computeCachedSiteLikelihood2(tree, branch_u, branch_v, rate, coalescenceRate, redCount, lineageCount, bHasDominantMarkers, dprint/*=false*/, iThread);
		} else {
			computeSiteLikelihood2(tree, branch_u, branch_v, rate, coalescenceRate, redCount, lineageCount, bHasDominantMarkers, dprint/*=false*/);
		}
