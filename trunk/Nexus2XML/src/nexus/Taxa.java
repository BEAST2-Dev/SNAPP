...
public class Taxa extends NexusBlock {
    ...
    public boolean equals(Taxa taxa) {
        ...
        for (int t = 1; t <= getNtax(); t++) {
            if (getLabel(t).equals(taxa.getLabel(t)) == false)
                return false;
        }
        return true;
    }
    ...
    public void setMustDetectLabels(boolean mustDetectLabels) {
        ...
        if (mustDetectLabels == false && (getOriginalTaxa() == null
    ...
    public static Taxa getInduced(Taxa origTaxa, TaxaSet hiddenTaxa) {
        ...
        try {
            ...
            for (int t = 1; t <= origTaxa.getNtax(); t++) {
                if (hiddenTaxa == null || hiddenTaxa.get(t) == false) {
                    count++;
    ...
    public void hideTaxa(TaxaSet hiddenTaxa) {
        ...
        if (inducedTaxa.equals(this) == false)
            try {
    ...
    public void hideAdditionalTaxa(TaxaSet additionalHidden) throws GenericException {
        ...
        int count = 0;
        for (int oID = 1; oID <= ntax; oID++) {
            if ((hiddenTaxa == null || hiddenTaxa.get(oID) == false)
                    && (additionalHidden == null || additionalO.get(oID) == false)) {
                ++count;
            }
        }
        setNtax(count);
