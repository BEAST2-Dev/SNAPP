...
namespace Phylib {
	...
	private:
		...
		typedef memory_manager<phylo_node> memory_manager_type;
		
	protected:
		link_type _header;
		link_type first_leaf;
		static int number_of_phylos;
		static memory_manager< phylo_node> manager;
	...
	public:
		...
		~phylo() {
			erase(_header);
			//cout<<"destroying "<<_header<<"leaving "<<number_of_phylos<<" trees left"<<endl;
			manager.put_node(_header);
			if (--number_of_phylos == 0)
	...
	template <typename T>
	int phylo<T>::number_of_phylos = 0;
	
	template <typename T>
	memory_manager< typename phylo<T>::phylo_node> phylo<T>::manager;
