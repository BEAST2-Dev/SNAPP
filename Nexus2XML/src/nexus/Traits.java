...
public class Traits extends NexusBlock {
    ...
    public int getTraitNumber(String traitName) {
        Integer col = (Integer) this.traitNumbers.get(traitName);
        if (col != null)
            return col.intValue();
        else
            return 0;
    ...
    public void hideTaxa(Taxa origTaxa, TaxaSet hiddenTaxa) {
        ...
        int count = 0;
        for (int j = 1; j <= this.nTraits; j++)
            matrix[0][j] = originalTraits.matrix[0][j];
        for (int t = 1; t <= origTaxa.getNtax(); t++) {
            if (hiddenTaxa == null || !hiddenTaxa.get(t)) {
                count++;
                for (int j = 1; j <= this.nTraits; j++)
                    matrix[count][j] = originalTraits.matrix[t][j];
            }
        }
    ...
    public void read(NexusStreamParser np, Taxa taxa)
        ...
        try {
            ...
            traitNumbers = new HashMap<String, Integer>();
            for (int col = 1; col <= nTraits; col++)
                traitNumbers.put(matrix[0][col], new Integer(col));
