...
import snap.likelihood.SnAPLikelihoodCoreT;
...
//	public Input<Tree> m_pTree = new Input<Tree>("tree", "tree with phylogenetic relations");
	...
	public Input<Boolean> m_usenNonPolymorphic = new Input<Boolean>("non-polymorphic", 
			...
			true);
	public Input<IntVector<PositiveInt>> ascSiteCountInput = new Input<>("ascSiteCount", "Counts for number of ascertained sites");
	public Input<IntScalar<PositiveInt>> totalSiteCountInput = new Input<>("totalSiteCount","Number of sites including those removed by ascertainment");
	...
	IntVector<PositiveInt> ascSiteCount;
	...
	public void initAndValidate() {
    	...
    	if (m_substitutionmodel.thetaInput.get() != null) {
	    	...
	    	Input<RealVectorParam<PositiveReal>> thetaInput = m_substitutionmodel.thetaInput;
			...
			if (m_bInitFromTree.get() == true) {
		    	...
		    	List<Double> sValues = thetaInput.get().getElements();
			...
			RealVector<PositiveReal> pTheta = thetaInput.get();
			RealVectorParam<PositiveReal> theta = new RealVectorParam<PositiveReal>();
			theta.initByName("value", sTheta, "upper", pTheta.getUpper(), "lower", pTheta.getLower(), "dimension", values.length);
			theta.setID(((BEASTInterface)pTheta).getID());
			thetaInput.get().assignFrom(theta);
    	} else {
    		// parameterised as coalescentRates
        	Input<RealVectorParam<PositiveReal>> coalescenceRatenput = m_substitutionmodel.m_pCoalescenceRate;
    		...
    		if (m_bInitFromTree.get() == true) {
    	    	...
    	    	List<Double> sValues = coalescenceRatenput.get().getElements();
    	        for (int i = 0; i < values.length; i++) {
    	            values[i] = new Double(sValues.get(i % sValues.size()));
    		...
    		RealVector<PositiveReal> pCoalescenceRate = coalescenceRatenput.get();
    		RealVectorParam<PositiveReal> coalescenceRate = new RealVectorParam<PositiveReal>();
    		...
    		coalescenceRate.setID(((BEASTInterface)pCoalescenceRate).getID());
...
//	    	}
	    	...
	    	double u = m_substitutionmodel.m_pU.get().get();
	    	double v  = m_substitutionmodel.m_pV.get().get();
			...
			if (!m_bUsenNonPolymorphic) {
				...
				if (ascSiteCount != null) {   
					ascLogP = (double)ascSiteCount.get(0) * Math.log(m_fP0) +
							  (double)ascSiteCount.get(1) * Math.log(m_fP1);
					logP += ascLogP;
				} else {
...
//	    			dprint /*= false*/);
    ...
    protected boolean requiresRecalculation() {
    	...
    	if (InputUtil.isDirty(ascSiteCountInput)) {
    		isDirty = true;
    	}
    	return isDirty;
    }
    ...
    private Double[] getCoalescentRates() {
		...
		if (m_substitutionmodel.m_pCoalescenceRate.get() != null) {
			getCoalescentRates = m_substitutionmodel.m_pCoalescenceRate.get().getElements().toArray(new Double[] {});
		} else {
			getCoalescentRates = m_substitutionmodel.thetaInput.get().getElements().toArray(new Double[] {});
