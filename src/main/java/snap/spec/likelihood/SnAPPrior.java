...
public class SnAPPrior extends Distribution {
    public Input<RealScalar<PositiveReal>> m_pAlpha = new Input<>("alpha", "Alpha parameter for the gamma prior on population size (theta) values", Validate.REQUIRED);
    public Input<RealScalar<PositiveReal>> m_pBeta = new Input<>("beta", "Beta parameter for the gamma prior on population size (theta) values", Validate.REQUIRED);
    public Input<RealScalar<PositiveReal>> m_pKappa = new Input<>("kappa", "prior parameter -- see docs for details");
    public Input<RealVector<PositiveReal>> m_pCoalescenceRate = new Input<>("coalescenceRate", "Populations sizes for the nodes in the tree", Validate.REQUIRED);
    public Input<RealScalar<PositiveReal>> m_pLambda = new Input<>("lambda", "Birth rate for the Yule model prior on the species tree");//, Validate.REQUIRED);
   ...
   public Input<Priors> m_priors = new Input<Priors>("rateprior", "prior on rates. " +
    ...
    public void initAndValidate() {
    	if (m_pKappa.get() == null) {
    		...
    		m_pKappa.setValue(new RealScalarParam<>(1.0, PositiveReal.INSTANCE), this);
    	}
    ...
    public double calculateLogP() {
        ...
        double alpha = m_pAlpha.get().get();
        double beta = m_pBeta.get().get();
        double kappa = m_pKappa.get().get();
        
        if (outsideBounds(m_pAlpha.get()) || outsideBounds(m_pBeta.get()) || outsideBounds(m_pKappa.get()) || 
        		outsideBounds(m_pCoalescenceRate.get())) {
        ...
        double lambda = m_pLambda.get().get();
        ...
        RealVector<PositiveReal> coalescenceRate = m_pCoalescenceRate.get();
		...
		if (PRIORCHOICE == 0) {
			...
			for (int iNode = 0; iNode < coalescenceRate.size(); iNode++) {
				double r = coalescenceRate.get(iNode);
			...
			//> let x be the root.
	        //> r = rate for node x.
    		double r = coalescenceRate.get(tree.getRoot().getNr());
			...
			*/
	        ...
	        for (int iNode = 0; iNode < tree.getNodeCount(); iNode++) {
	        	...
	        	if (!node.isRoot()) {
	        		...
	        		r = coalescenceRate.get(node.getNr());
					
					
					double r0 = coalescenceRate.get(parent.getNr());
    ...
    private boolean outsideBounds(Tensor<?,?> realParameter) {
    	if (realParameter == null) {
    		return false;
    	}
    	return (realParameter.isValid());
