...
public class ScaleOperator extends Operator {
    ...
    public Input<RealVectorParam<PositiveReal>> m_pParameter = new Input<>("parameter", "if specified, this parameter is scaled",
    ...
    public double proposal() {
		...
		**/
        ...
        RealVectorParam<PositiveReal> param = m_pParameter.get();
        int dim = param.size();
        ...
        if (bScaleAllIndependently) {
            ...
            for (int i = 0; i < dim; i++) {
                ...
                double value = scaleOne * param.get(i);
            ...
            for (int i = 0; i < dim; i++) {
                param.set(i, param.get(i) * scale);
            }

            for (int i = 0; i < dim; i++) {
                if (param.get(i) < param.getLower() ||
                        param.get(i) > param.getUpper()) {
                    return Double.NEGATIVE_INFINITY;
...
//            }
            ...
            double oldValue = param.get(index);
